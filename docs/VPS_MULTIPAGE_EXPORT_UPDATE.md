# VPS Multi-Page Export Update

This document contains the updated VPS code for your `pdf-export-service` GitHub repository.

## Summary

These updates add two new endpoints:
1. `/export-multipage` - Exports a multi-page Polotno scene to a single PDF
2. `/export-labels` - Exports + imposes labels onto sheets (using qpdf to preserve vectors)

## Dockerfile Updates

Add `qpdf` to the Dockerfile:
...
Replace your entire `server.js` with this updated version:
...
const { jsonToPDF, jsonToPDFBase64 } = require('@polotno/pdf-export');
...
# Should now show: "qpdf": "qpdf version X.X.X"
```
