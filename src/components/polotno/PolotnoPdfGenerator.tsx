...
export function PolotnoPdfGenerator({
  ...
  const getProgressPercentage = (): number => {
    ...
    // Weight phases: preparing 5%, exporting 65%, uploading 15%, composing 15%
    // (converting phase only shows for fallback raster export)
    ...
    switch (progress.phase) {
      case 'preparing':
        return 5;
      case 'exporting':
        return 5 + basePercent * 0.65;
      case 'converting':
        // Only shows for raster fallback path
        return 70 + basePercent * 0.10;
      case 'uploading':
        return 80 + basePercent * 0.10;
      case 'composing':
        return 90 + basePercent * 0.10;
      default:
        return basePercent;
    }
  };
                ...
                <Progress value={getProgressPercentage()} className="h-3" />
                    <div className="flex justify-between text-xs text-muted-foreground">
                      <span>
                        ...
                        {progress.phase === 'converting' && `Converting to CMYK (fallback)...`}
